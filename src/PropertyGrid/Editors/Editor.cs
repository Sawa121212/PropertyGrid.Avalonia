        ...
        internal ControlTemplate GetEditorTemplate(object template)
        {
            ...
            if (template is string)
            {
                var genericStyle = Application.Current.Styles.OfType<StyleInclude>().FirstOrDefault(styleInclude => styleInclude.
                             Source.AbsoluteUri.StartsWith("avares://PropertyGrid/ControlsToolKit.xaml"));
                var nextLevelStyle = (genericStyle.Loaded as Styles)?.OfType<StyleInclude>()
                    .FirstOrDefault(style => style.Source.AbsoluteUri.EndsWith("Generic.xaml"));

                var result = (nextLevelStyle.Loaded as Styles)?.OfType<StyleInclude>()
                        .FirstOrDefault(styleInclude => styleInclude.
                             Source.AbsoluteUri.EndsWith("PropertyGrid/Controls/Styles/Editor/EditorResources.xaml"));
