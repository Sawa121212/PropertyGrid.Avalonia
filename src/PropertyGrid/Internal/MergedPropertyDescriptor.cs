...
namespace PropertyGrid.Internal
{
    ...
    internal class MergedPropertyDescriptor : PropertyDescriptor
    {
        ...
        protected override void OnValueChanged(object component, System.EventArgs e)
        {
            if (internalValueSet)
                return;
        ...
        private class MultiMergeCollection : ICollection, IEnumerable
        {
            ...
            public bool MergeCollection(ICollection newCollection)
            {
                if (!locked)
                {
                    ...
                    {
                        if (((array[i] == null) != (items[i] == null)) || ((items[i] != null) && !items[i].Equals(array[i])))
                        {
                            items = new object[0];
                            return false;
                        }
